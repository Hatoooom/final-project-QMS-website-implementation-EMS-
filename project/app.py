...
def get_state():
    ...
    cur.close()
    conn.close()
    return jsonify({'ambulances': ambs, 'inventory': inv, 'logs': logs})
...
def handle_incident():
    ...
    if action == 'create':
        cur.execute("INSERT INTO incidents (id, t0, status) VALUES (%s, NOW(), 'PENDING') ON CONFLICT DO NOTHING", (data['id'],))
        ...
        # 🔴 CHANGED: Health drops by 15 per trip now (was 5). Fuel drops by 20.
        cur.execute("UPDATE ambulances SET status = 'IDLE', supplies_ok = FALSE, fuel = fuel - 20, health = health - 15 WHERE id = %s", (data['unit_id'],))
